...
This module provides the (not on stand-alone working) class OBReader which defines a kind of interface for classes
used to read in limit order book data and return the relevant parts as numpy arrays. Note that no specific format of
the data is required and thus, OBReader does not provide full functionability. This should be provided by a suitable
subclass for each specific data format. (see lobster.py for a reader for LOBSTERDATA files).
"""
...
class OBReader():
    ...
    def add_data(self, key_str, data, overwrite=True):
        ...
        if not overwrite and (key_str in self.data):
            warnings.warn("LOBSTERReader has already data stored with key {} and overwriting disabled.".format(key_str),
                          RuntimeWarning)
            return False
    ...
    def load_ordervolume(
            ...
            num_levels_calc_str="",
            write_output=False):
        ''' Extracts the volume of orders in the first num_level buckets at a uniform time grid of num_observations
        observations from the interval [time_start_calc, time_end_calc]. The volume process is extrapolated
        constantly on the last level in the file, for the case that time_end_calc is larger than the last time stamp
        in the file. If num_obersvations is None, then the volume will be extracted on the time grid given in the
        data '''
        ...
        if num_observations is None:
            time_stamps, volume_bid, volume_ask = self._load_ordervolume_full(num_levels_calc, profile2vol_fct=np.sum,
                                                                              ret_np=False)
            dt = 0
        else:
            dt, time_stamps, volume_bid, volume_ask = self._load_ordervolume(num_observations, num_levels_calc,
                                                                             profile2vol_fct=np.sum)
            self.add_data("time_discr_ov", dt)
    ...
    def load_volume_process(
        ...
        warnings.warn("Function load_volume_process will be removed soon. Please use load_ordervolume instead",
                      FutureWarning)
    ...
    def load_marketdepth(
        ...
        if num_observations is None:
            time_stamps, volume_bid, volume_ask = self._load_ordervolume_full(num_levels_calc, profile2vol_fct=np.mean,
                                                                              ret_np=False)
            dt = 0
        else:
            dt, time_stamps, depth_bid, depth_ask = self._load_ordervolume(num_observations, num_levels_calc,
...
def get_data_from_file(filename):
    ''' Loads the order book data from a csv file with format bidprofile; askprofile '''
    warnings.warn(
        "The method get_data_from_file is supposed to be removed in future versions. Use the OBReader function with "
